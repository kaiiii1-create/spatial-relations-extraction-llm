...
# Configure Ollama API and model name
OLLAMA_URL = "http://localhost:11434/api/generate"
OLLAMA_MODEL = "gemma:2b-instruct"


#  Path to the XML document folder
...
# Function to extract all text from XML
...
# Use Ollama with gemma model to extract spatial relationships
...
#  Main script to iterate through XML files and extract spatial relationships
results = []
...
for filename in tqdm(os.listdir(XML_DIR)):
    if filename.endswith(".xml"):
        ...
        if text:
            response = get_spatial_relationships(text[:1000]) 
